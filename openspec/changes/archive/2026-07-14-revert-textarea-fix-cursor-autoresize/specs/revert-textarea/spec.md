...
The system SHALL allow the user to click on a shortcut rendered in the preview to edit its keys and action inline. The action input SHALL be a multi-line `<textarea>` with auto-resize to content height and cursor positioned at end on activation. Keys and action SHALL be independently editable. Each field SHALL save on blur independently.
...
- **WHEN** the user edits the action textarea and clicks away
